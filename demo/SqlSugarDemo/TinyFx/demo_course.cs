...
namespace SqlSugarDemo.DAL
{
	...
	public class Demo_courseMO : MySqlTableMO<Demo_courseEO>
	{
		...
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`demo_course`";
		...
		public override int Add(Demo_courseEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Demo_courseEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Demo_courseEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`Year`, `CourseID`, `Name`, `OrderNum`) VALUE (@Year, @CourseID, @Name, @OrderNum);";
		...
		public int AddByBatch(IEnumerable<Demo_courseEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Demo_courseEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Demo_courseEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`Year`, `CourseID`, `Name`, `OrderNum`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"({item.Year},'{item.CourseID}','{item.Name}',{item.OrderNum}),");
				if (count == batchCount)
				{
					count = 0;
		...
		{
			sql_ = $"DELETE FROM {TableName} WHERE `Year` = @Year AND `CourseID` = @CourseID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		{
			sql_ = $"UPDATE {TableName} SET `Year` = @Year, `CourseID` = @CourseID, `Name` = @Name, `OrderNum` = @OrderNum WHERE `Year` = @Year_Original AND `CourseID` = @CourseID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@Year", item.Year, MySqlDbType.Year),
				Database.CreateInParameter("@CourseID", item.CourseID, MySqlDbType.VarChar),
		...
		{
			sql_ = $"UPDATE {TableName} SET `Name` = @Name  WHERE `Year` = @Year AND `CourseID` = @CourseID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		{
			sql_ = $"UPDATE {TableName} SET `OrderNum` = @OrderNum  WHERE `Year` = @Year AND `CourseID` = @CourseID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <return></return>
		public Demo_courseEO GetByPK(int year, string courseID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(year, courseID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Demo_courseEO.MapDataReader);
		}
		public async Task<Demo_courseEO> GetByPKAsync(int year, string courseID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(year, courseID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Demo_courseEO.MapDataReader);
		}
		private void RepairGetByPKData(int year, string courseID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false, TransactionManager tm_ = null)
		{
			...
			sql_ = BuildSelectSQL("`Year` = @Year AND `CourseID` = @CourseID", 0, null, null, isForUpdate_);
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@Year", year, MySqlDbType.Year),
				Database.CreateInParameter("@CourseID", courseID, MySqlDbType.VarChar),
