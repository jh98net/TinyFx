...
namespace SqlSugarDemo.DAL
{
	...
	public class Demo_userMO : MySqlTableMO<Demo_userEO>
	{
		...
		{
			sql_ = $"DELETE FROM {TableName} WHERE " + (classID != null ? "`ClassID` = @ClassID" : "`ClassID` IS NULL");
			paras_ = new List<MySqlParameter>();
			if (classID != null)
		...
		private void RepairGetByPKData(long userID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false, TransactionManager tm_ = null)
		{
