...
namespace TinyFx.Extensions.AWS.LoadBalancing
{
    internal class TargetGroupRegisterService
    {
        ...
        public TargetGroupRegisterService()
        {
            ...
            {
                _isRegister = section.LoadBalancing.RegisterTargetGroup;
                _groupName = section.LoadBalancing.TargetGroupName;
        ...
        public async Task Register()
        {
            ...
            var arn = await _groupDCache.GetOrLoadAsync(_groupName);
            if (!arn.HasValue)
            {
                using (var redLock = await RedisUtil.LockAsync($"__TargetGroup:{_groupName}"))
                {
                    if (!redLock.IsLocked)
                        throw new Exception($"TargetGroupRegisterService注册时TargetGroup不存在且无法申请锁.groupName:{_groupName}");
                    arn = await _groupDCache.GetOrLoadAsync(_groupName);
                    if (!arn.HasValue)
                    {
                        var vpcId = await _globalDCache.GetVpcId();
                        var group = await _lbSvc.CreateTargetGroup(vpcId, _groupName);
                        if (group == null)
                            throw new Exception($"TargetGroupRegisterService.Register时无法创建targetGroup。vpcId:{vpcId} groupName:{_groupName}");
                        _targetGroupArn = group.TargetGroupArn;
                        await _groupDCache.SetAsync(_groupName, _targetGroupArn);
                    }
                    else
