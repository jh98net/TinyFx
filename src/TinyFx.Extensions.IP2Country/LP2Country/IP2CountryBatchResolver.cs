...
namespace TinyFx.Extensions.IP2Country
{
    public class IP2CountryBatchResolver : IIP2CountryBatchResolver
    {
        ...
        public IP2CountryBatchResolver(IIP2CountryResolver resolver) => _resolver = resolver ?? throw new ArgumentNullException(nameof(resolver));
        ...
        public IIPRangeCountry[] Resolve(IEnumerable<string> ips) => Resolve(ips.ToArray());

        public IIPRangeCountry[] Resolve(IEnumerable<IPAddress> ips) => Resolve(ips.ToArray());

        public IDictionary<string, IIPRangeCountry> ResolveAsDictionary(string[] ips) => ResolveAsDictionary(ips.AsEnumerable());

        public IDictionary<IPAddress, IIPRangeCountry> ResolveAsDictionary(IPAddress[] ips) => ResolveAsDictionary(ips.AsEnumerable());
