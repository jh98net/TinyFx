        ...
        public SymmetricProvider(SymmetricMode symmetric, byte[] key, byte[] iv = null)
        {
            switch (symmetric)
            {
                case SymmetricMode.DES:
                    if (key.Length != 8)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，DES算法密钥长度为8字节。");
                    break;
                case SymmetricMode.RC2:
                    if (key.Length < 1 || key.Length > 128)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，RC2算法密钥长度为1-128字节。");
                    break;
                case SymmetricMode.Rijndael:
                    if (key.Length != 16 && key.Length != 24 && key.Length != 32)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，Rijndael算法密钥长度为16字节，24字节或者32字节。");
                    break;
                case SymmetricMode.TripleDES:
                    if (key.Length != 16 && key.Length != 24)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，TripleDES算法密钥长度为16字节或24字节。");
                    break;
                case SymmetricMode.AES:
                    if (key.Length != 16 && key.Length != 24 && key.Length != 32)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，AES算法密钥长度为16字节或24字节或32字节。");
                    break;
                default:
