...
namespace TinyFx.Security
{
    ...
    public class SymmetricProvider
    {
        ...
        /// 获取或设置运算模式
        /// </summary>
        public CipherMode Mode { get { return _mode; } set { _mode = value; } }

        private PaddingMode _padding = PaddingMode.PKCS7;
        /// <summary>
        /// 获取或设置填充模式
        /// </summary>
        public PaddingMode Padding { get { return _padding; } set { _padding = value; } }
        /// <summary>
        /// 密文的编码格式
        /// </summary>
        ...
        public SymmetricProvider(SymmetricMode symmetric, string key, byte[] iv = null, CipherEncode cipherEncode = CipherEncode.Base64, Encoding encoding = null)
            : this(symmetric, (encoding ?? Encoding.UTF8).GetBytes(key), iv, encoding: encoding)
        { }
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        /// <param name="cipherEncode">编码格式处理</param>
        /// <param name="encoding">字符集</param>
        public SymmetricProvider(SymmetricMode symmetric, byte[] key, byte[] iv = null, CipherEncode cipherEncode = CipherEncode.Base64, Encoding encoding = null)
        {
            switch (symmetric)
            {
                case SymmetricMode.DES:
                    if (key.Length != 8)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，DES算法密钥长度为8Byte。");
                    break;
                case SymmetricMode.RC2:
                    if (key.Length > 16 || key.Length < 5)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，RC2算法密钥长度为5Byte-16Byte。");
                    break;
                case SymmetricMode.Rijndael:
                    if (key.Length != 16 && key.Length != 24 && key.Length != 32)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，Rijndael算法密钥长度为16byte，24byte或者32byte。");
                    break;
                case SymmetricMode.TripleDES:
                    if (key.Length != 16 && key.Length != 24)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，TripleDES算法密钥长度为16Byte或24Byte。");
                    break;
                case SymmetricMode.AES:
                    if (key.Length != 128 && key.Length != 192 && key.Length != 256)
                        throw new ArgumentOutOfRangeException("key", "密钥长度错误，AES算法密钥长度为128Byte或192Byte或256Byte。");
                    break;
                default:
        ...
        {
            return GetProvider().CreateEncryptor();
        }

        private ICryptoTransform GetDecryptor()
        {
            return GetProvider().CreateDecryptor();
        }
        #endregion
