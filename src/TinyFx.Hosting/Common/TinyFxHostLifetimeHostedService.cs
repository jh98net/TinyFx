...
namespace TinyFx
{
    internal class TinyFxHostLifetimeHostedService : IHostedService
    {
        ...
        {
            _logger.LogInformation($"[{ConfigUtil.Project.ProjectId}] OnStarted被调用");
            // 此处代码在Host启动后执行
        ...
        {
            _logger.LogInformation($"[{ConfigUtil.Project.ProjectId}] OnStopping被调用");
            // 此处代码在Host停止动作开始时执行
        ...
        {
            _logger.LogInformation($"[{ConfigUtil.Project.ProjectId}] OnStopped被调用");
            // 此处代码在Host停止后执行
